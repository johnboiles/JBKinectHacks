...
void SendMoveEvent(double x, double y, double z) {
	stringstream ss;
	ss << "\"x\":"  << x << ",\"y\":" << y << ",\"z\":" << z;
	SendEvent("Move", ss.str());
}

void ProcessUser(XnUserID player) {
	if (!g_UserGenerator.GetSkeletonCap().IsTracking(player))
	{
		cerr << "ERROR: Not tracking user" << endl;
		return;
	}
	...
	double positionZ = ((rightHandPosition.position.Z + armLength) - (headPosition.position.Z - 200))/(armLength * 2);
	...
	if (time - g_time > 100) {
		...
		double positionX = ((rightHandPosition.position.X - boxOriginX) / boxWidth);
		double positionY = 1 - ((rightHandPosition.position.Y - boxOriginY) / boxHeight);

		SendMoveEvent(positionX, positionY, positionZ);
		g_time = time;
	}
}
